---
title: "Clinical Trial Data Validation Across Languages and Tools"
author: "Ronald G. Thomas"
date: "2025-01-15"
categories: [clinical-trials, data-cleaning, r, lua, javascript]
description: "I did not really understand how many layers of validation sit between a clinical trial data entry form and a reliable analysis dataset until I started mapping out EDC edit checks, open-source tools, and the possibility of generating JavaScript validation from a simple spreadsheet processed by Lua."
...
![A fountain pen resting across three notebooks, each open to a page in a different handwriting style: one validation check expressed consistently across different languages.
...
alternatives, and general-purpose languages. It ended
with a surprisingly elegant pipeline that starts with a
...
We walk through what each layer involves and compare
the available tooling. We then build toward a novel
pipeline where non-programmers define validation
rules in a CSV file, Lua processes those rules into
JavaScript functions, and a Shiny application loads
the generated JavaScript for real-time form
validation. The Lua pipeline is the most
architecturally distinctive component and appears
...
1. Catalog the ten major categories of EDC
...
![Three differently-shaped keys on a single ring, resting on a technical drawing: one credential, multiple compatible systems.](media/images/ambiance1.jpg){.img-fluid height=3.5in}
...
map to standardized medical dictionaries. An adverse
...
learning to prioritize data review. Sites with higher
...
organizational context, budget, and the degree of
...
For larger datasets or centralized validation, AJAX
...
centralized rule management across multiple
applications:
...
1. **Security of generated JavaScript.** The Lua
   ...
   Sanitize inputs before code generation.
...
](media/images/ambiance3.png){.img-fluid height=3.5in}
...
- The Lua pipeline is presented as a proof of concept.
  ...
  sanitization, error handling, logging, and
  integration testing.
...
1. Build a complete working prototype of the Lua
   pipeline with input sanitization, error handling,
...
integrity failure mode. Second, R (`validate`,
`pointblank`), Python (`pandera`), and SQL provide
flexible validation for custom analysis pipelines.
Third, Shiny supports three tiers of form
validation: `validate()`/`need()`, `shinyvalidate`,
and client-side JavaScript. Fourth, a Lua-generated
JavaScript pipeline can bridge the gap between
non-programmer rule definition and real-time
browser validation.

For those working with clinical trial data
validation, I would be interested to hear which
combination of tools has proved effective in
practice.
...
- [Running ZZedc Independently for Clinical Research Data
  Management](../cln-zzedc-investigator-independence/)
...
1. **Post 95: Clinical Trial Data Validation Across Languages** (this post)
